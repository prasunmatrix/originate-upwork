...
class Projectclient_model extends MX_Controller {
	...
	public function getProjects($organization_id,$member_id='',$start = '', $limit = '',$count=FALSE,$where=array()){ 
		$arr=array(
				'select'=>'p.project_id,p.project_url,p.project_title,p.project_posted_date,p.project_expired_date,p.project_status,p_s.is_hourly,p_s.is_fixed,p_s.budget,p_s.is_visible_anyone',
				'table'=>'project as p',
				'join'=>array(
		...
		$arr['where']['p_o.organization_id']=$organization_id;
		if($where){
