...
class Test_three extends MX_Controller {
   ...
   private $data;
	...
	public function __construct(){
		...
		$this->load->model('test_three_model', 'test_three');
		$this->data['table'] = 'test_three';
	...
	public function list_record(){
		...
		$offset = 20;
		$this->data['main_title'] = 'Test Three Management';
		$this->data['second_title'] = 'All Test Three List';
		$this->data['title'] = 'Test Three';
		$breadcrumb = array(
			array(
				'name' => 'Test Three',
				'path' => '',
		...
		$this->data['list'] = $this->test_three->getList($srch, $limit, $offset);
		$this->data['list_total'] = $this->test_three->getList($srch, $limit, $offset, FALSE);
	...
	public function load_ajax_page(){
		...
		if($page == 'add'){
			...
			$this->data['detail'] = $this->test_three->getDetail($id);
	...
	public function add(){
		if(post() && $this->input->is_ajax_request()){
			...
			if($this->form_validation->run()){
				$post = post();
				$insert = $this->test_three->addRecord($post);
	...
	public function edit(){
		if(post() && $this->input->is_ajax_request()){
			...
			if($this->form_validation->run()){
				...
				$ID = post('ID');
				unset($post['ID']);
				$update = $this->test_three->updateRecord($post, $ID);
				$this->api->cmd('reload');
			}else{
				$errors = validation_errors_array();
				$this->api->set_error($errors);
