...
class Testimonial_model extends CI_Model{
	...
	public function getList($srch=array(), $limit=0, $offset=20, $for_list=TRUE){
		...
		if(!empty($srch['show']) && $srch['show'] == 'trash'){
			$this->db->where('a.testimonial_status', DELETE_STATUS);	
		}else{
			$this->db->where('a.testimonial_status <>', DELETE_STATUS);	
		}
	...
	public function addRecord($data=array()){
		$structure = array(
			...
			'testimonial_status' => !empty($data['status']) ? $data['status'] : '0',
		);
		$ins['data'] = $structure;
	...
	public function deleteRecord($id=''){
		if($id && is_array($id)){
			return $this->db->where_in($this->primary_key, $id)->update($this->table, array('testimonial_status' => DELETE_STATUS));
		}else{
			$ins['data'] = array('testimonial_status' => DELETE_STATUS);
			$ins['table'] = $this->table;
			$ins['where'] = array($this->primary_key => $id);
			return  update($ins);
		}
	...
	public function getDetail($id=''){
		...
		foreach($lang_result as $k => $v){
			...
			$lang_company_name[$v['lang']] = $v['company_name'];
			$lang_description[$v['lang']] = $v['description'];
		}
		$result['lang'] = array();
		foreach($result as $k => $v){
			$result['lang']['name'] = $lang_name;
			$result['lang']['company_name'] = $lang_company_name;
	...
	public function get_all_testimonial(){
		...
		$this->db->select('*')
			->from('testimonial a')
			->join('testimonial_names b', 'a.testimonial_code=b.testimonial_code');
		
		$this->db->where('a.testimonial_status', ACTIVE_STATUS);	
