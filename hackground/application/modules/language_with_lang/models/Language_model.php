...
class Language_model extends CI_Model{
	...
	public function getList($srch=array(), $limit=0, $offset=20, $for_list=TRUE){
		...
		if(!empty($srch['term'])){
			$this->db->group_start();
			$this->db->like('b.language_title', $srch['term']);
			$this->db->or_like('a.language_key', $srch['term']);
			$this->db->group_end();
		}
	...
	public function addRecord($data=array()){
		$structure = array(
			'language_key' => !empty($data['name']) ? $data['name'] : '',
	...
	public function getDetail($id=''){
		...
		foreach($lang_result as $k => $v){
			$lang_name[$v['lang']] = $v['language_title'];
		}
		$result['lang'] = array();
		foreach($result as $k => $v){
			$result['lang']['language_title'] = $lang_name;
		}
		return $result;
	}
