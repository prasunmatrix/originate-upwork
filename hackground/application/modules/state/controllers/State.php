...
class State extends MX_Controller {
   ...
   private $data;
	...
	public function __construct(){
		...
		$this->data['lang_table'] = 'state_names';
		...
		$this->state->configure($model_configuration);
	...
	public function list_record(){
		...
		$offset = 20;
		$this->data['main_title'] = 'State Management';
		$this->data['second_title'] = 'All State List';
		...
		$this->data['list'] = $this->state->getList($srch, $limit, $offset);
		$this->data['list_total'] = $this->state->getList($srch, $limit, $offset, FALSE);
	...
	public function load_ajax_page(){
		...
		if($page == 'add'){
			...
			$this->data['detail'] = $this->state->getDetail($id);
	...
	public function add(){
		...
		if(post() && $this->input->is_ajax_request()){
			...
			foreach($lang as $k => $v){
				$this->form_validation->set_rules('lang[state_name]['.$v.']', "name $v", 'required|trim|max_length[100]');
			}
			$this->form_validation->set_rules('state_key', 'slug', 'required|regex_match[/^[a-z\-A-Z]+$/]|is_unique[state.state_key]');
			$this->form_validation->set_rules('status', 'status', '');
			if($this->form_validation->run()){
				$post = post();
				$insert = $this->state->addRecord($post);
	...
	public function edit(){
		...
		if(post() && $this->input->is_ajax_request()){
			...
			if(post('state_key_old') == post('state_key')){
				$this->form_validation->set_rules('state_key', 'slug', 'required|regex_match[/^[a-z\-A-Z]+$/]');
			}else{
				$this->form_validation->set_rules('state_key', 'slug', 'required|regex_match[/^[a-z\-A-Z]+$/]|is_unique[state.state_key]');
			}
			...
			if($this->form_validation->run()){
				...
				$ID = post('ID');
				unset($post['ID']);
				$update = $this->state->updateRecord($post, $ID);
				$this->api->cmd('reload');
			}else{
				$errors = validation_errors_array();
				$this->api->set_error($errors);
	...
	public function change_status(){
		if(post() && $this->input->is_ajax_request()){
			...
			if(is_array($ID)){
				$this->db->where_in($this->data['primary_key'], $ID)->update($this->data['table'], array('state_status' => $sts));
			}else{
				$upd['data'] = array('state_status' => $sts);
	...
	public function upload_file($type=''){
		if($_FILES && $this->input->is_ajax_request()){
			...
			if(! $this->upload->do_upload('file')){
				...
				$this->api->data('file_url', UPLOAD_HTTP_PATH.'state-icon/'.$pathupload.$this->upload->data('file_name'));
