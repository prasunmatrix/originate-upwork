...
class State_model extends CI_Model{
	...
	public function getList($srch=array(), $limit=0, $offset=20, $for_list=TRUE){
		...
		if(!empty($srch['term'])){
			$this->db->like('b.state_name', $srch['term']);
		}
	...
	public function addRecord($data=array()){
		$structure = array(
			'state_key' => !empty($data['state_key']) ? $data['state_key'] : '',
			'country_code' => !empty($data['country_code']) ? $data['country_code'] : '',
			'state_order' => !empty($data['state_order']) ? $data['state_order'] : '',
			'state_thumb' => !empty($data['state_thumb']) ? $data['state_thumb'] : '',
			'state_status' => !empty($data['status']) ? $data['status'] : '0',
		);
		$ins['data'] = $structure;
	...
	public function deleteRecord($id=''){
		if($id && is_array($id)){
			return $this->db->where_in($this->primary_key, $id)->update($this->table, array('state_status' => DELETE_STATUS));
		}else{
			$ins['data'] = array('state_status' => DELETE_STATUS);
			$ins['table'] = $this->table;
			$ins['where'] = array($this->primary_key => $id);
			return  update($ins);
		}
	...
	public function getDetail($id=''){
		...
		foreach($lang_result as $k => $v){
			$lang_name[$v['state_lang']] = $v['state_name'];
		}
		$result['lang'] = array();
		foreach($result as $k => $v){
			$result['lang']['state_name'] = $lang_name;
		}
		return $result;
	}
