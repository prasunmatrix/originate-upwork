              ...
              <table class="table table-hover">
				...
				<tr>
                  ...
                  <td class="text-right" style="padding-right:20px;">
					...
					<a href="<?php echo JS_VOID; ?>" onclick="add_sub_category('<?php echo $v[$primary_key]; ?>')" data-toggle="tooltip" title="Add Child"><i class="fa fa-plus <?php echo ICON_SIZE;?>"></i></a>
					&nbsp;
					<a href="<?php echo base_url('sub_category_level_four/list_record/'.$v[$primary_key]); ?>" data-toggle="tooltip" title="View Child"><i class="fa fa-list-ul <?php echo ICON_SIZE;?>"></i></a>
					&nbsp;
					
					<?php if($this->sub_category_level_three->hasNoChild($v[$primary_key])){ ?>
					
					<a href="<?php echo JS_VOID; ?>" data-toggle="tooltip" onclick="manage_component('<?php echo $v[$primary_key]; ?>')" title="Manage Component"><i class="fa fa-mouse-pointer <?php echo ICON_SIZE;?>"></i></a>
					&nbsp;
					<?php } ?>
...
function manage_component(id){
	var url = '<?php echo base_url('component/load_ajax_page?page=add_category_component&category_level=3&category_subchild_id=');?>'+id;
	load_ajax_modal(url);
}

function add_sub_category(cat_id){
	var url = '<?php echo base_url('sub_category_level_four/load_ajax_page?page='.$add_command);?>&category_subchild_level_3_id='+cat_id;
	load_ajax_modal(url);
}

function add(){
	var url = '<?php echo base_url($curr_controller.'load_ajax_page?page='.$add_command.'&category_subchild_id='.$category_id);?>';
	load_ajax_modal(url);
}
