...
class Sub_category_level_four extends MX_Controller {
   ...
   private $data;
	...
	public function __construct(){
		...
		$this->load->model('sub_category_level_four_model', 'sub_category_level_four');
		$this->data['table'] = 'category_subchild_level_4';
		$this->data['lang_table'] = 'category_subchild_level_4_names';
		...
		$this->sub_category_level_four->configure($model_configuration);
	...
	public function list_record($category_id=''){
		...
		$category_subchild_level_3_id = $category_id;
		$category_subchild_id = getField('category_subchild_id', 'category_subchild_level_3', 'category_subchild_level_3_id', $category_subchild_level_3_id);
		$parent_category_id = getField('category_id', 'category_subchild', 'category_subchild_id', $category_subchild_id);
		
		$this->data['category_id'] = $category_id;
		$this->data['category_name'] = $category_name = $this->sub_category_level_three_model->get_category_name($category_id);
		$this->data['category_subchild_name'] = $category_subchild_name = $this->sub_category_model->get_category_name($category_subchild_id);
		...
		$breadcrumb = array(
			...
			array(
				'name' => "$category_subchild_name",
				'path' => base_url('sub_category_level_three/list_record/'.$category_subchild_id),
			),
			array(
				'name' => "$category_name",
				'path' => '',
		...
		$this->data['list'] = $this->sub_category_level_four->getList($srch, $limit, $offset);
		$this->data['list_total'] = $this->sub_category_level_four->getList($srch, $limit, $offset, FALSE);
	...
	public function load_ajax_page(){
		...
		if($page == 'add'){
			$this->data['category_subchild_level_3_id'] = get('category_subchild_level_3_id');
			if($this->data['category_subchild_level_3_id']){
				$this->data['category_name'] =$this->sub_category_level_three_model->get_category_name($this->data['category_subchild_level_3_id']);
			}else{
			...
			$this->data['detail'] = $this->sub_category_level_four->getDetail($id);
	...
	public function add(){
		...
		if(post() && $this->input->is_ajax_request()){
			...
			$this->form_validation->set_rules('category_subchild_key', 'category key', 'required|regex_match[/^[a-z\-A-Z]+$/]|is_unique[category_subchild_level_4.category_subchild_key]');
			if($this->form_validation->run()){
				$post = post();
				$insert = $this->sub_category_level_four->addRecord($post);
	...
	public function edit(){
		...
		if(post() && $this->input->is_ajax_request()){
			...
			if($this->form_validation->run()){
				...
				$ID = post('ID');
				unset($post['ID']);
				$update = $this->sub_category_level_four->updateRecord($post, $ID);
				$this->api->cmd('reload');
			}else{
				$errors = validation_errors_array();
				$this->api->set_error($errors);
	...
	public function delete_record($id=''){
		...
		if($id){
			$this->sub_category_level_four->deleteRecord($id);
			$cmd = get('cmd');
			if($cmd && $cmd == 'remove'){
				if($id && is_array($id)){
