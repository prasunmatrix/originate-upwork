...
class Badge_model extends CI_Model{
	...
	public function getList($srch=array(), $limit=0, $offset=20, $for_list=TRUE){
		...
		if($for_list){
			$result = $this->db->limit($offset, $limit)->order_by('a.display_order', 'ASC')->order_by('a.'.$this->primary_key, 'ASC')->get()->result_array();
			if($result){
				foreach($result as $k => $v){
					$result[$k]['icon_image_url'] = UPLOAD_HTTP_PATH.'badge-icons/'.$v['icon_image'];
				}
			}
		}else{
	...
	public function addRecord($data=array()){
		$structure = array(
			'icon_image' => !empty($data['icon_image']) ? $data['icon_image'] : '',
	...
	public function getAllBadges(){
		...
		$this->db->where('b.lang', $admin_default_lang);	
		$result = $this->db->order_by('a.display_order', 'ASC')->get()->result_array();
		if($result){
			foreach($result as $k => $v){
				$result[$k]['icon_image_url'] = UPLOAD_HTTP_PATH.'badge-icons/'.$v['icon_image'];
			}
