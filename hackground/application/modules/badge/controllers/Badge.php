...
class Badge extends MX_Controller {
   ...
   private $data;
	...
	public function __construct(){
		...
		$this->load->model('badge_model', 'badge');
		$this->data['table'] = 'badges';
		$this->data['lang_table'] = 'badges_names';
		$this->data['primary_key'] = 'badge_id';
		...
		$this->badge->configure($model_configuration);
	...
	public function list_record(){
		...
		$offset = 20;
		$this->data['main_title'] = 'Badge Management';
		$this->data['second_title'] = 'All Badge List';
		...
		$this->data['list'] = $this->badge->getList($srch, $limit, $offset);
		$this->data['list_total'] = $this->badge->getList($srch, $limit, $offset, FALSE);
	...
	public function load_ajax_page(){
		...
		if($page == 'add'){
			...
			$this->data['detail'] = $this->badge->getDetail($id);
			$this->data['title'] = 'Edit Badge';
		}
	...
	public function add(){
		...
		if(post() && $this->input->is_ajax_request()){
			...
			if($this->form_validation->run()){
				$post = post();
				$insert = $this->badge->addRecord($post);
	...
	public function edit(){
		...
		if(post() && $this->input->is_ajax_request()){
			...
			if($this->form_validation->run()){
				...
				$ID = post('ID');
				unset($post['ID']);
				$update = $this->badge->updateRecord($post, $ID);
				$this->api->cmd('reload');
			}else{
				$errors = validation_errors_array();
				$this->api->set_error($errors);
	...
	public function upload_file($type=''){
		if($_FILES && $this->input->is_ajax_request()){
			$upload_dir = UPLOAD_PATH.'badge-icons';
			...
			if(! $this->upload->do_upload('file')){
				...
				$this->api->data('file_url', UPLOAD_HTTP_PATH.'badge-icons/'.$this->upload->data('file_name'));
