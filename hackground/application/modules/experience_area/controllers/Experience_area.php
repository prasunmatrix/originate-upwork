...
class Experience_area extends MX_Controller {
   ...
   private $data;
	...
	public function __construct(){
		...
		$this->load->model('experience_area_model', 'experience_area');
		$this->data['table'] = 'option_experience_area';
		$this->data['lang_table'] = 'option_experience_area_names';
		$this->data['primary_key'] = 'experience_area_id';
		...
		$this->experience_area->configure($model_configuration);
	...
	public function list_record(){
		...
		$offset = 20;
		$this->data['main_title'] = 'Experience Area Management';
		$this->data['second_title'] = 'All Experience Area List';
		$this->data['title'] = 'Experience Area';
		$breadcrumb = array(
			array(
		...
		$this->data['list'] = $this->experience_area->getList($srch, $limit, $offset);
		$this->data['list_total'] = $this->experience_area->getList($srch, $limit, $offset, FALSE);
		...
		$this->data['add_btn'] = 'Add Experience Area';
	...
	public function load_ajax_page(){
		...
		$this->data['page'] = $page;
		if($page == 'add'){
			$this->data['title'] = 'Add Experience Area';
			...
			$this->data['detail'] = $this->experience_area->getDetail($id);
			$this->data['title'] = 'Edit Experience Area';
		}
	...
	public function add(){
		...
		if(post() && $this->input->is_ajax_request()){
			...
			if($this->form_validation->run()){
				$post = post();
				$insert = $this->experience_area->addRecord($post);
	...
	public function edit(){
		...
		if(post() && $this->input->is_ajax_request()){
			...
			if($this->form_validation->run()){
				...
				$ID = post('ID');
				unset($post['ID']);
				$update = $this->experience_area->updateRecord($post, $ID);
				$this->api->cmd('reload');
			}else{
				$errors = validation_errors_array();
				$this->api->set_error($errors);
	...
	public function delete_record($id=''){
		...
		if($id){
			$this->experience_area->deleteRecord($id);
			$cmd = get('cmd');
			if($cmd && $cmd == 'remove'){
				if($id && is_array($id)){
