...
class Commission_slab extends MX_Controller {
   ...
   private $data;
	...
	public function __construct(){
		...
		$this->load->model('commission_slab_model', 'commission_slab');
		$this->data['table'] = 'commission_slab';
		$this->data['primary_key'] = 'commission_slab_id';
		parent::__construct();
	...
	public function list_record(){
		...
		$offset = 20;
		$this->data['main_title'] = 'Commission Slab Management';
		$this->data['second_title'] = 'All Slab List';
		$this->data['title'] = 'Commission Slab';
		$breadcrumb = array(
			array(
		...
		$this->data['list'] = $this->commission_slab->getList($srch, $limit, $offset);
		$this->data['list_total'] = $this->commission_slab->getList($srch, $limit, $offset, FALSE);
	...
	public function load_ajax_page(){
		...
		$this->data['page'] = $page;
		if($page == 'add'){
			$this->data['title'] = 'Add Commission Slab';
			...
			$this->data['detail'] = $this->commission_slab->getDetail($id);
			$this->data['title'] = 'Edit Commission Slab';
		}
	...
	public function add(){
		if(post() && $this->input->is_ajax_request()){
			...
			$this->form_validation->set_rules('min_value', 'min_value', 'required|trim');
			$this->form_validation->set_rules('max_value', 'max_value', 'required|trim');
			$this->form_validation->set_rules('commission_percent', 'commission_percent', 'required|trim');
			$this->form_validation->set_rules('status', 'status', '');
			if($this->form_validation->run()){
				$post = post();
				$insert = $this->commission_slab->addRecord($post);
	...
	public function edit(){
		if(post() && $this->input->is_ajax_request()){
			...
			if($this->form_validation->run()){
				...
				$ID = post('ID');
				unset($post['ID']);
				$update = $this->commission_slab->updateRecord($post, $ID);
				$this->api->cmd('reload');
			}else{
				$errors = validation_errors_array();
				$this->api->set_error($errors);
	...
	public function delete_record($id=''){
		...
		if($id){
			$this->commission_slab->deleteRecord($id);
			$cmd = get('cmd');
			if($cmd && $cmd == 'remove'){
				if($id && is_array($id)){
