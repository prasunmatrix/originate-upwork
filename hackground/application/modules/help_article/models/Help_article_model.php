...
class Help_article_model extends CI_Model{
	...
	public function getList($srch=array(), $limit=0, $offset=20, $for_list=TRUE){
		...
		if(!empty($srch['term'])){
			$this->db->like('b.title', $srch['term']);
		}
		
		if(!empty($srch['cms_help_id'])){
			$this->db->where('a.cms_help_id', $srch['cms_help_id']);
		}
	...
	public function addRecord($data=array()){
		$structure = array(
			...
			'cms_help_id' => !empty($data['cms_help_id']) ? $data['cms_help_id'] : '0',
	...
	public function getDetail($id=''){
		...
		foreach($lang_result as $k => $v){
			$lang_name[$v['lang']] = $v['title'];
			$lang_description[$v['lang']] = $v['description'];
		}
		$result['lang'] = array();
		foreach($result as $k => $v){
			$result['lang']['title'] = $lang_name;
