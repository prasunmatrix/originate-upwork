...
class Notification_template extends MX_Controller {
   ...
   private $data;
	...
	public function __construct(){
		...
		$this->load->model('notification_template_model', 'notification_template');
		$this->data['table'] = 'notifications_template';
		$this->data['lang_table'] = 'notifications_template_names';
		$this->data['primary_key'] = 'notification_template_id';
		...
		$this->notification_template->configure($model_configuration);
	...
	public function list_record(){
		...
		$offset = 20;
		$this->data['main_title'] = 'Notification Template Management';
		$this->data['second_title'] = 'All Notification Template List';
		$this->data['title'] = 'Notification Template';
		$breadcrumb = array(
			array(
		...
		$this->data['list'] = $this->notification_template->getList($srch, $limit, $offset);
		$this->data['list_total'] = $this->notification_template->getList($srch, $limit, $offset, FALSE);
		...
		$this->data['add_btn'] = 'Add Template';
	...
	public function export_csv(){
		...
		$srch = get();
		$file_name = "Notification Template List -".date('d M Y').".csv";
		if(!empty($srch['export']) && $srch['export'] == 1){
			$array = array();
			$array[] = array("ID", "Template For", "Template Key", "Status");
			$list = $this->notification_template->getList($srch, 0, 5000);
			if($list){
				foreach($list as $k => $v){
					$status = '';
					if($v['status'] == '1'){
					...
					$array[] = array($v['notification_template_id'], $v['template_for'], $v['template_key'], $status);
				}
			}
	...
	public function load_ajax_page(){
		...
		if($page == 'add'){
			...
			$this->data['detail'] = $this->notification_template->getDetail($id);
	...
	public function add(){
		...
		if(post() && $this->input->is_ajax_request()){
			...
			foreach($lang as $k => $v){
				$this->form_validation->set_rules('lang[template_content]['.$v.']', "content $v", 'required|trim');
			}
			
			$this->form_validation->set_rules('template_key', 'template key', 'required|regex_match[/^[a-z\-A-Z_]+$/]|is_unique[notifications_template.template_key]');
			$this->form_validation->set_rules('status', 'status', '');
			$this->form_validation->set_rules('template_for', 'template for', 'required');
			if($this->form_validation->run()){
				$post = post();
				$insert = $this->notification_template->addRecord($post);
	...
	public function edit(){
		...
		if(post() && $this->input->is_ajax_request()){
			...
			$this->form_validation->set_rules('template_key', 'template key', 'required|regex_match[/^[a-z\-A-Z_]+$/]');
			$this->form_validation->set_rules('status', 'status', '');
			...
			if($this->form_validation->run()){
				...
				$ID = post('ID');
				unset($post['ID']);
				$update = $this->notification_template->updateRecord($post, $ID);
				$this->api->cmd('reload');
			}else{
				$errors = validation_errors_array();
				$this->api->set_error($errors);
	...
	public function delete_record($id=''){
		...
		if($id){
			$this->notification_template->deleteRecord($id);
			$cmd = get('cmd');
			if($cmd && $cmd == 'remove'){
				if($id && is_array($id)){
