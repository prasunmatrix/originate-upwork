...
class Help_cms_model extends CI_Model{
	...
	public function getList($srch=array(), $limit=0, $offset=20, $for_list=TRUE){
		...
		if(!empty($srch['parent'])){
			$this->db->where('a.parent_id', $srch['parent']);
		}else{
	...
	public function addRecord($data=array()){
		$structure = array(
			'cms_help_slug' => !empty($data['cms_help_slug']) ? $data['cms_help_slug'] : '',
