...
class Delivery_times extends MX_Controller {
   ...
   private $data;
	...
	public function __construct(){
		...
		$this->load->model('delivery_times_model', 'delivery_times');
		$this->data['table'] = 'delivery_times';
		$this->data['lang_table'] = 'delivery_times_names';
		$this->data['primary_key'] = 'delivery_id';
		...
		$this->delivery_times->configure($model_configuration);
	...
	public function list_record(){
		...
		$offset = 20;
		$this->data['main_title'] = 'Delivery Times Management';
		$this->data['second_title'] = 'All Delivery Times List';
		$this->data['title'] = 'Delivery Times';
		$breadcrumb = array(
			array(
				'name' => 'Delivery Times',
				'path' => '',
		...
		$this->data['list'] = $this->delivery_times->getList($srch, $limit, $offset);
		$this->data['list_total'] = $this->delivery_times->getList($srch, $limit, $offset, FALSE);
		...
		$this->data['add_btn'] = 'Add Delivery Times';
	...
	public function load_ajax_page(){
		...
		$this->data['page'] = $page;
		if($page == 'add'){
			$this->data['title'] = 'Add Delivery Times';
			...
			$this->data['detail'] = $this->delivery_times->getDetail($id);
			$this->data['title'] = 'Edit Delivery Times';
		}
	...
	public function add(){
		...
		if(post() && $this->input->is_ajax_request()){
			...
			foreach($lang as $k => $v){
				$this->form_validation->set_rules('lang[delivery_title]['.$v.']', "name $v", 'required|trim|max_length[100]');
				$this->form_validation->set_rules('lang[delivery_proposal_title]['.$v.']', "proposal name $v", 'required|trim|max_length[100]');
			}
			
			$this->form_validation->set_rules('status', 'status', '');
			if($this->form_validation->run()){
				$post = post();
				$insert = $this->delivery_times->addRecord($post);
	...
	public function edit(){
		...
		if(post() && $this->input->is_ajax_request()){
			...
			if($this->form_validation->run()){
				...
				$ID = post('ID');
				unset($post['ID']);
				$update = $this->delivery_times->updateRecord($post, $ID);
				$this->api->cmd('reload');
			}else{
				$errors = validation_errors_array();
				$this->api->set_error($errors);
	...
	public function delete_record($id=''){
		...
		if($id){
			$this->delivery_times->deleteRecord($id);
			$cmd = get('cmd');
			if($cmd && $cmd == 'remove'){
				if($id && is_array($id)){
