...
class Delivery_times_model extends CI_Model{
	...
	public function getList($srch=array(), $limit=0, $offset=20, $for_list=TRUE){
		...
		if(!empty($srch['term'])){
			$this->db->group_start();
			$this->db->like('b.delivery_title', $srch['term']);
			$this->db->or_like('a.delivery_proposal_title', $srch['term']);
			$this->db->group_end();
		}
	...
	public function getDetail($id=''){
		...
		$lang_name=$lang_proposal_title=array();
		
		foreach($lang_result as $k => $v){
			$lang_name[$v['lang']] = $v['delivery_title'];
			$lang_proposal_title[$v['lang']] = $v['delivery_proposal_title'];
		}
		$result['lang'] = array();
		foreach($result as $k => $v){
			$result['lang']['delivery_title'] = $lang_name;
			$result['lang']['delivery_proposal_title'] = $lang_proposal_title;
		}
		return $result;
	}
	...
	public function get_all_data(){
		...
		$this->db->select('*')
			...
			->join('delivery_times_names b', 'a.delivery_id=b.delivery_id');
		
		$this->db->where('a.status', ACTIVE_STATUS);	
