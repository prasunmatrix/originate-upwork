...
class Email_template extends MX_Controller {
   ...
   private $data;
	...
	public function __construct(){
		...
		$this->data['table'] = 'mailtemplate';
		$this->data['lang_table'] = 'mailtemplate_names';
		$this->data['primary_key'] = 'template_id';
		...
		$this->email_template->configure($model_configuration);
	...
	public function list_record(){
		...
		$offset = 20;
		$this->data['main_title'] = 'Email Template Management';
		$this->data['second_title'] = 'All Email Template List';
		...
		$this->data['list'] = $this->email_template->getList($srch, $limit, $offset);
		$this->data['list_total'] = $this->email_template->getList($srch, $limit, $offset, FALSE);
	...
	public function export_csv(){
		...
		$srch = get();
		$file_name = "Email Template List -".date('d M Y').".csv";
		if(!empty($srch['export']) && $srch['export'] == 1){
			$array = array();
			$array[] = array("ID", "Template For", "Template Key", "Status");
			$list = $this->email_template->getList($srch, 0, 5000);
			if($list){
				foreach($list as $k => $v){
					$status = '';
					if($v['status'] == '1'){
					...
					$array[] = array($v['template_id'], $v['template_for'], $v['template_type'], $status);
				}
			}
	...
	public function load_ajax_page(){
		...
		if($page == 'add'){
			...
			$this->data['detail'] = $this->email_template->getDetail($id);
	...
	public function add(){
		...
		if(post() && $this->input->is_ajax_request()){
			...
			foreach($lang as $k => $v){
				$this->form_validation->set_rules('lang[template_subject]['.$v.']', "subject $v", 'required|trim');
				$this->form_validation->set_rules('lang[template_content]['.$v.']', "content $v", 'required|trim');
			}
			
			$this->form_validation->set_rules('template_type', 'template type', 'required|regex_match[/^[a-z\-A-Z]+$/]');
			$this->form_validation->set_rules('status', 'status', '');
			if($this->form_validation->run()){
				$post = post();
				$insert = $this->email_template->addRecord($post);
	...
	public function edit(){
		...
		if(post() && $this->input->is_ajax_request()){
			...
			if($this->form_validation->run()){
				...
				$ID = post('ID');
				unset($post['ID']);
				$update = $this->email_template->updateRecord($post, $ID);
				$this->api->cmd('reload');
			}else{
				$errors = validation_errors_array();
				$this->api->set_error($errors);
	...
	public function delete_record($id=''){
		...
		if($id){
			$this->email_template->deleteRecord($id);
			$cmd = get('cmd');
			if($cmd && $cmd == 'remove'){
				if($id && is_array($id)){
