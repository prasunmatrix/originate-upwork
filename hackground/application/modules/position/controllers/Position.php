...
class Position extends MX_Controller {
   ...
   private $data;
	...
	public function __construct(){
		...
		$this->data['table'] = 'option_position';
		$this->data['lang_table'] = 'option_position_names';
		$this->data['primary_key'] = 'position_id';
		...
		$this->position->configure($model_configuration);
	...
	public function list_record(){
		...
		$offset = 20;
		$this->data['main_title'] = 'Position Management';
		$this->data['second_title'] = 'All Position List';
		$this->data['title'] = 'Position';
		$breadcrumb = array(
			array(
				'name' => 'Position',
				'path' => '',
		...
		$this->data['list'] = $this->position->getList($srch, $limit, $offset);
		$this->data['list_total'] = $this->position->getList($srch, $limit, $offset, FALSE);
		...
		$this->data['add_btn'] = 'Add Position';
	...
	public function load_ajax_page(){
		...
		if($page == 'add'){
			...
			$this->data['detail'] = $this->position->getDetail($id);
	...
	public function add(){
		...
		if(post() && $this->input->is_ajax_request()){
			...
			if($this->form_validation->run()){
				$post = post();
				$insert = $this->position->addRecord($post);
	...
	public function edit(){
		...
		if(post() && $this->input->is_ajax_request()){
			...
			if($this->form_validation->run()){
				...
				$ID = post('ID');
				unset($post['ID']);
				$update = $this->position->updateRecord($post, $ID);
				$this->api->cmd('reload');
			}else{
				$errors = validation_errors_array();
				$this->api->set_error($errors);
