...
class Country extends MX_Controller {
   ...
   private $data;
	...
	public function __construct(){
		...
		$this->data['table'] = 'country';
		$this->data['lang_table'] = 'country_names';
		$this->data['primary_key'] = 'country_code';
		...
		$this->country->configure($model_configuration);
	...
	public function list_record(){
		...
		$offset = 20;
		$this->data['main_title'] = 'Country Management';
		$this->data['second_title'] = 'All Country List';
		...
		$this->data['list'] = $this->country->getList($srch, $limit, $offset);
		$this->data['list_total'] = $this->country->getList($srch, $limit, $offset, FALSE);
	...
	public function load_ajax_page(){
		...
		if($page == 'add'){
			...
			$this->data['detail'] = $this->country->getDetail($id);
			$this->data['title'] = 'Edit Country';
		}
	...
	public function add(){
		...
		if(post() && $this->input->is_ajax_request()){
			...
			foreach($lang as $k => $v){
				$this->form_validation->set_rules('lang[country_name]['.$v.']', "name $v", 'required|trim|max_length[100]');
			}
			
			$this->form_validation->set_rules('country_code', 'country code', 'required|alpha|is_unique[country.country_code]');
			$this->form_validation->set_rules('status', 'status', '');
			if($this->form_validation->run()){
				$post = post();
				$insert = $this->country->addRecord($post);
	...
	public function edit(){
		...
		if(post() && $this->input->is_ajax_request()){
			...
			if($this->form_validation->run()){
				...
				$ID = post('ID');
				unset($post['ID']);
				$update = $this->country->updateRecord($post, $ID);
				$this->api->cmd('reload');
			}else{
				$errors = validation_errors_array();
				$this->api->set_error($errors);
	...
	public function change_status(){
		if(post() && $this->input->is_ajax_request()){
			...
			if(is_array($ID)){
				$this->db->where_in($this->data['primary_key'], $ID)->update($this->data['table'], array('country_status' => $sts));
			}else{
				$upd['data'] = array('country_status' => $sts);
