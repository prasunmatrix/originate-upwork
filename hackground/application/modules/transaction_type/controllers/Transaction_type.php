...
class Transaction_type extends MX_Controller {
   ...
   private $data;
	...
	public function __construct(){
		...
		$this->load->model('transaction_type_model', 'transaction_type');
		$this->data['table'] = 'wallet_transaction_type';
	...
	public function list_record(){
		...
		$offset = 20;
		$this->data['main_title'] = 'Transaction Type Management';
		$this->data['second_title'] = 'All Transaction Type List';
		$this->data['title'] = 'Transaction Type';
		$breadcrumb = array(
			array(
		...
		$this->data['list'] = $this->transaction_type->getList($srch, $limit, $offset);
		$this->data['list_total'] = $this->transaction_type->getList($srch, $limit, $offset, FALSE);
	...
	public function load_ajax_page(){
		...
		$this->data['page'] = $page;
		if($page == 'add'){
			$this->data['title'] = 'Add Transaction Type';
			...
			$this->data['detail'] = $this->transaction_type->getDetail($id);
			$this->data['title'] = 'Edit Transaction Type';
		}
	...
	public function add(){
		if(post() && $this->input->is_ajax_request()){
			...
			$this->form_validation->set_rules('title_tkey', 'key', 'required|trim');
			$this->form_validation->set_rules('description_tkey', 'description', 'required|trim');
			if($this->form_validation->run()){
				$post = post();
				$insert = $this->transaction_type->addRecord($post);
	...
	public function edit(){
		if(post() && $this->input->is_ajax_request()){
			...
			if($this->form_validation->run()){
				...
				$ID = post('ID');
				unset($post['ID']);
				$update = $this->transaction_type->updateRecord($post, $ID);
				$this->api->cmd('reload');
			}else{
				$errors = validation_errors_array();
				$this->api->set_error($errors);
	...
	public function delete_record($id=''){
		...
		if($id){
			$this->transaction_type->deleteRecord($id);
			$cmd = get('cmd');
			if($cmd && $cmd == 'remove'){
				if($id && is_array($id)){
