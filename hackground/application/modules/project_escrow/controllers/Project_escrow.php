...
class Project_escrow extends MX_Controller {
   ...
   private $data;
	...
	public function __construct(){
		...
		$this->load->model('project_escrow_model', 'project_escrow');
		$this->data['table'] = 'project_contract_milestone';
		$this->data['primary_key'] = 'contract_milestone_id';
		parent::__construct();
	...
	public function list_record(){
		...
		$offset = 20;
		$this->data['main_title'] = 'Project Escrow';
		$this->data['second_title'] = 'All Escrow List';
		$this->data['title'] = 'Project Escrow';
		$breadcrumb = array(
			array(
				'name' => 'Project Escrow',
				'path' => '',
		...
		$this->data['list'] = $this->project_escrow->getList($srch, $limit, $offset);
		$this->data['list_total'] = $this->project_escrow->getList($srch, $limit, $offset, FALSE);
		
		if(!empty($srch['project_id']) || !empty($srch['contract_id'])){
			$this->data['credit_total'] = $this->project_escrow->getTotal($srch, 'credit');
			$this->data['debit_total'] = $this->project_escrow->getTotal($srch, 'debit');
			$this->data['remaining_total'] = ($this->data['credit_total'] - $this->data['debit_total']);
			$this->data['show_total'] = TRUE;
		}else{
			$this->data['show_total'] = FALSE;
		}
	...
	public function load_ajax_page(){
		...
		if($page == 'add'){
			...
			$this->data['detail'] = $this->project_escrow->getDetail($id);
	...
	public function add(){
		if(post() && $this->input->is_ajax_request()){
			...
			if($this->form_validation->run()){
				$post = post();
				$insert = $this->project_escrow->addRecord($post);
	...
	public function edit(){
		if(post() && $this->input->is_ajax_request()){
			...
			if($this->form_validation->run()){
				...
				$ID = post('ID');
				unset($post['ID']);
				$update = $this->project_escrow->updateRecord($post, $ID);
				$this->api->cmd('reload');
			}else{
				$errors = validation_errors_array();
				$this->api->set_error($errors);
	...
	public function delete_record($id=''){
		...
		if($id){
			$this->project_escrow->deleteRecord($id);
			$cmd = get('cmd');
			if($cmd && $cmd == 'remove'){
				if($id && is_array($id)){
