...
class Nationality extends MX_Controller {
   ...
   private $data;
	...
	public function __construct(){
		...
		$this->data['table'] = 'option_nationality';
		$this->data['lang_table'] = 'option_nationality_names';
		$this->data['primary_key'] = 'nationality_id';
		...
		$this->nationality->configure($model_configuration);
	...
	public function list_record(){
		...
		$offset = 20;
		$this->data['main_title'] = 'Nationality Management';
		$this->data['second_title'] = 'All Nationality List';
		$this->data['title'] = 'Nationality';
		$breadcrumb = array(
			array(
				'name' => 'Nationality',
				'path' => '',
		...
		$this->data['list'] = $this->nationality->getList($srch, $limit, $offset);
		$this->data['list_total'] = $this->nationality->getList($srch, $limit, $offset, FALSE);
		...
		$this->data['add_btn'] = 'Add Nationality';
	...
	public function load_ajax_page(){
		...
		$this->data['page'] = $page;
		if($page == 'add'){
			$this->data['title'] = 'Add Nationality';
			...
			$this->data['detail'] = $this->nationality->getDetail($id);
			$this->data['title'] = 'Edit Nationality';
		}
	...
	public function add(){
		...
		if(post() && $this->input->is_ajax_request()){
			...
			if($this->form_validation->run()){
				$post = post();
				$insert = $this->nationality->addRecord($post);
	...
	public function edit(){
		...
		if(post() && $this->input->is_ajax_request()){
			...
			if($this->form_validation->run()){
				...
				$ID = post('ID');
				unset($post['ID']);
				$update = $this->nationality->updateRecord($post, $ID);
				$this->api->cmd('reload');
			}else{
				$errors = validation_errors_array();
				$this->api->set_error($errors);
