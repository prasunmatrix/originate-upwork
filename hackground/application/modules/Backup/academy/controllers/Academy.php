...
class Academy extends MX_Controller {
   ...
   private $data;
	...
	public function __construct(){
		...
		$this->data['table'] = 'option_academy';
		$this->data['lang_table'] = 'option_academy_names';
		$this->data['primary_key'] = 'academy_id';
		...
		$this->academy->configure($model_configuration);
	...
	public function list_record(){
		...
		$offset = 20;
		$this->data['main_title'] = 'Academy Management';
		$this->data['second_title'] = 'All Academy List';
		...
		$this->data['list'] = $this->academy->getList($srch, $limit, $offset);
		$this->data['list_total'] = $this->academy->getList($srch, $limit, $offset, FALSE);
	...
	public function load_ajax_page(){
		...
		if($page == 'add'){
			...
			$this->data['detail'] = $this->academy->getDetail($id);
			$this->data['title'] = 'Edit Academy';
		}
	...
	public function add(){
		...
		if(post() && $this->input->is_ajax_request()){
			...
			if($this->form_validation->run()){
				$post = post();
				$insert = $this->academy->addRecord($post);
	...
	public function edit(){
		...
		if(post() && $this->input->is_ajax_request()){
			...
			if($this->form_validation->run()){
				...
				$ID = post('ID');
				unset($post['ID']);
				$update = $this->academy->updateRecord($post, $ID);
				$this->api->cmd('reload');
			}else{
				$errors = validation_errors_array();
				$this->api->set_error($errors);
