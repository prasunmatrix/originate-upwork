...
class Career_level extends MX_Controller {
   ...
   private $data;
	...
	public function __construct(){
		...
		$this->load->model('career_level_model', 'career_level');
		$this->data['table'] = 'option_career_level';
		$this->data['lang_table'] = 'option_career_level_names';
		$this->data['primary_key'] = 'career_level_id';
		...
		$this->career_level->configure($model_configuration);
	...
	public function list_record(){
		...
		$offset = 20;
		$this->data['main_title'] = 'Career Level Management';
		$this->data['second_title'] = 'All Career Level List';
		$this->data['title'] = 'Career Level';
		$breadcrumb = array(
			array(
				'name' => 'Career Level',
				'path' => '',
		...
		$this->data['list'] = $this->career_level->getList($srch, $limit, $offset);
		$this->data['list_total'] = $this->career_level->getList($srch, $limit, $offset, FALSE);
		...
		$this->data['add_btn'] = 'Add Career Level';
	...
	public function load_ajax_page(){
		...
		$this->data['page'] = $page;
		if($page == 'add'){
			$this->data['title'] = 'Add Career Level';
			...
			$this->data['detail'] = $this->career_level->getDetail($id);
			$this->data['title'] = 'Edit Career Level';
		}
	...
	public function add(){
		...
		if(post() && $this->input->is_ajax_request()){
			...
			if($this->form_validation->run()){
				$post = post();
				$insert = $this->career_level->addRecord($post);
	...
	public function edit(){
		...
		if(post() && $this->input->is_ajax_request()){
			...
			if($this->form_validation->run()){
				...
				$ID = post('ID');
				unset($post['ID']);
				$update = $this->career_level->updateRecord($post, $ID);
				$this->api->cmd('reload');
			}else{
				$errors = validation_errors_array();
				$this->api->set_error($errors);
	...
	public function delete_record($id=''){
		...
		if($id){
			$this->career_level->deleteRecord($id);
			$cmd = get('cmd');
			if($cmd && $cmd == 'remove'){
				if($id && is_array($id)){
