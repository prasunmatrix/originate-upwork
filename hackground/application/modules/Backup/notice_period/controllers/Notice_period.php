...
class Notice_period extends MX_Controller {
   ...
   private $data;
	...
	public function __construct(){
		...
		$this->load->model('notice_period_model', 'notice_period');
		$this->data['table'] = 'option_notice_period';
		$this->data['lang_table'] = 'option_notice_period_names';
		$this->data['primary_key'] = 'notice_period_id';
		...
		$this->notice_period->configure($model_configuration);
	...
	public function list_record(){
		...
		$offset = 20;
		$this->data['main_title'] = 'Notice Period Management';
		$this->data['second_title'] = 'All Notice Period List';
		$this->data['title'] = 'Notice Period';
		$breadcrumb = array(
			array(
				'name' => 'Notice Period',
				'path' => '',
		...
		$this->data['list'] = $this->notice_period->getList($srch, $limit, $offset);
		$this->data['list_total'] = $this->notice_period->getList($srch, $limit, $offset, FALSE);
		...
		$this->data['add_btn'] = 'Add Notice Period';
	...
	public function load_ajax_page(){
		...
		$this->data['page'] = $page;
		if($page == 'add'){
			$this->data['title'] = 'Add Notice Period';
			...
			$this->data['detail'] = $this->notice_period->getDetail($id);
			$this->data['title'] = 'Edit Notice Period';
		}
	...
	public function add(){
		...
		if(post() && $this->input->is_ajax_request()){
			...
			if($this->form_validation->run()){
				$post = post();
				$insert = $this->notice_period->addRecord($post);
	...
	public function edit(){
		...
		if(post() && $this->input->is_ajax_request()){
			...
			if($this->form_validation->run()){
				...
				$ID = post('ID');
				unset($post['ID']);
				$update = $this->notice_period->updateRecord($post, $ID);
				$this->api->cmd('reload');
			}else{
				$errors = validation_errors_array();
				$this->api->set_error($errors);
	...
	public function delete_record($id=''){
		...
		if($id){
			$this->notice_period->deleteRecord($id);
			$cmd = get('cmd');
			if($cmd && $cmd == 'remove'){
				if($id && is_array($id)){
