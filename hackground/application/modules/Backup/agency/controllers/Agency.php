...
class Agency extends MX_Controller {
   ...
   private $data;
	...
	public function __construct(){
		...
		$this->data['table'] = 'realestate_agent';
		$this->data['primary_key'] = 'agent_id';
		...
		$this->agency->configure($model_configuration);
	...
	public function add(){
		...
		if(post() && $this->input->is_ajax_request()){
			...
			$this->form_validation->set_rules('agent_registration_number', 'registration number', 'required|is_unique[realestate_agent.agent_registration_number]');
	...
	public function change_status(){
		if(post() && $this->input->is_ajax_request()){
			...
			if(is_array($ID)){
				$this->db->where_in($this->data['primary_key'], $ID)->update($this->data['table'], array('agent_status' => $sts));
			}else{
