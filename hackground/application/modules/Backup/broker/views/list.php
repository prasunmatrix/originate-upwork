      ...
      <div class="box">
		...
		<div class="box-body table-responsive no-padding" id="main_table">
              <table class="table table-hover">
				...
				$status = '';
				if($v['broker_status'] == ACTIVE_STATUS){
				...
				}else if($v['broker_status'] == INACTIVE_STATUS){
				...
				<tr>
                  ...
                  <td><?php echo $v['member_name']; ?><br>
                  <b>User Id : <?php echo $v['member_id']; ?></b></td>
                  <td><?php 
                  if($v['broker_number']){
				  	 echo 'Reg:'.$v['broker_number'].'<br>';
				  }
                 if($v['company_telephone']){
				  	 echo 'Tel:'.$v['company_telephone'].'<br>';
				  }
				  if($v['broker_name']){
				  	 echo 'Agent Name: '.$v['broker_name'].'<br>';
				  }
                   ?></td>
                  <td><?php echo $v['agent_name']; ?></td>
                  <td>
                  <?php if(!empty($v['broker_document'])){ 
                  $document=json_decode($v['broker_document']);
                  if($document){
                  	$docN=0;
                  ...
                  <td class="text-right" style="padding-right:20px;">
					<?php if($v['broker_status'] != DELETE_STATUS){ ?>
