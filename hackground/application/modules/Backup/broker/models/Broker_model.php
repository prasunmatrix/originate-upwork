...
class Broker_model extends CI_Model{
	...
	public function getList($srch=array(), $limit=0, $offset=20, $for_list=TRUE){
		...
		$this->db->select('a.*,m.member_name,b.agent_name')
			->from($this->table. ' a')
			->join('member m', 'a.member_id='.'m.member_id')
			->join('realestate_agent b', 'a.agent_id='.'b.agent_id');
		
		if(!empty($srch['show']) && $srch['show'] == 'trash'){
			$this->db->where('a.broker_status', DELETE_STATUS);	
		}else{
			$this->db->where('a.broker_status <>', DELETE_STATUS);	
		}
		if(!empty($srch['agent_id'])){
			$this->db->where('b.agent_id', $srch['agent_id']);	
		}
		if(!empty($srch['term'])){
			$this->db->group_start();
		...
		if($for_list){
			$result = $this->db->limit($offset, $limit)->order_by('a.broker_id', 'DESC')->get()->result_array();
		}else{
