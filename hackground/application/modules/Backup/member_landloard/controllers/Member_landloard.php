...
class Member_landloard extends MX_Controller {
   ...
   private $data;
	...
	public function __construct(){
		...
		$this->load->model('member_landloard_model', 'member_landloard');
		$this->data['table'] = 'member_landlord';
		$this->data['primary_key'] = 'landlord_id';
		parent::__construct();
	...
	public function list_record(){
		...
		$offset = 20;
		$this->data['main_title'] = 'Landlord Management';
		$this->data['second_title'] = 'All Landlord List';
		$this->data['title'] = 'Landlord';
		$breadcrumb = array(
			array(
				'name' => 'Landlord',
				'path' => '',
		...
		$this->data['list'] =$this->member_landloard->getList($srch, $limit, $offset);
		$this->data['list_total'] =$this->member_landloard->getList($srch, $limit, $offset, FALSE);
	...
	public function load_ajax_page(){
		...
		if($page == 'add'){
			...
			$this->data['detail'] =$this->member_landloard->getDetail($id);
			$this->data['title'] = 'Edit Landlord';
		}
	...
	public function add(){
		if(post() && $this->input->is_ajax_request()){
			...
			if($this->form_validation->run()){
				$post = post();
				$insert =$this->member_landloard->addRecord($post);
	...
	public function edit(){
		if(post() && $this->input->is_ajax_request()){
			$this->load->library('form_validation');
			$this->form_validation->set_rules('landlord_name', 'name', 'required|trim|max_length[100]');
			$this->form_validation->set_rules('deed_number', 'deed number', 'required|trim');
			$this->form_validation->set_rules('pre_registration_no', 'registration no', 'required|trim');
			$this->form_validation->set_rules('status', 'status', '');
			...
			if($this->form_validation->run()){
				...
				$ID = post('ID');
				unset($post['ID']);
				$update =$this->member_landloard->updateRecord($post, $ID);
				$this->api->cmd('reload');
			}else{
				$errors = validation_errors_array();
				$this->api->set_error($errors);
	...
	public function change_status(){
		if(post() && $this->input->is_ajax_request()){
			...
			if(is_array($ID)){
				$this->db->where_in($this->data['primary_key'], $ID)->update($this->data['table'], array('landlord_status' => $sts));
			}else{
				$upd['data'] = array('landlord_status' => $sts);
	...
	public function delete_record($id=''){
		...
		if($id){
			$this->member_landloard->deleteRecord($id);
			$cmd = get('cmd');
			if($cmd && $cmd == 'remove'){
				if($id && is_array($id)){
