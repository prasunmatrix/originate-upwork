      ...
      <div class="box">
		...
		<div class="box-body table-responsive no-padding" id="main_table">
              <table class="table table-hover">
				...
				$status = '';
				if($v['component_status'] == ACTIVE_STATUS){
				...
				}else if($v['component_status'] == INACTIVE_STATUS){
				...
				<tr>
                  ...
                  <td><?php echo $v['component_name']; ?></td>
                  <td><?php echo $v['description']; ?></td>
                  <td><?php echo $status; ?></td>
                  <td class="text-right" style="padding-right:20px;">
					<?php if($v['component_status'] != DELETE_STATUS){ ?>
					
					<?php if($v['component_type'] !== 'TEXT'){ ?>
					<a href="<?php echo JS_VOID; ?>" onclick="add_component_value('<?php echo $v['component_id']; ?>')" data-toggle="tooltip" title="Add Values"><i class="icon-feather-plus <?php echo ICON_SIZE;?>"></i></a>
					&nbsp;
					<a href="<?php echo base_url('component/show_values/'.$v['component_id']); ?>" data-toggle="tooltip" title="View Values"><i class="icon-feather-list <?php echo ICON_SIZE;?>"></i></a>
					&nbsp;
					<?php } ?>
...
function add_component_value(id){
	if(!id){
		return false;
	}
	var url = '<?php echo base_url($curr_controller.'load_ajax_page?page=add_component_value');?>&component_id='+id;
	load_ajax_modal(url);
}
