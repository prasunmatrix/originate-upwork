      ...
      <div class="box">
		...
		<div class="box-body table-responsive no-padding" id="main_table">
              <table class="table table-hover">
				...
				$status = '';
				if($v['component_value_status'] == ACTIVE_STATUS){
				...
				}else if($v['component_value_status'] == INACTIVE_STATUS){
				...
				<tr>
                  ...
                  <td><?php echo $v['component_value_name']; ?></td>
                  <td><?php echo $status; ?></td>
                  <td class="text-right" style="padding-right:20px;">
					<?php if($v['component_value_status'] != DELETE_STATUS){ ?>
...
function add(){
	var url = '<?php echo base_url($curr_controller.'load_ajax_page?page='.$add_command.'&component_id='.$component_id);?>';
	load_ajax_modal(url);
}
...
function deleteRecord(id, permanent){
	...
	if(c){
		console.log('ok');
		var url = '<?php echo base_url($curr_controller.'delete_values');?>/'+id;
		if(permanent){
			url += '?cmd=remove';
		}
...
function changeStatus(sts, id, ele){
	...
	if(status.indexOf(sts) !== -1){
		var url = '<?php echo base_url($curr_controller.'change_value_status');?>';
		$.ajax({
			url : url,
