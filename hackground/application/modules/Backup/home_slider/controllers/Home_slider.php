...
class Home_slider extends MX_Controller {
   ...
   private $data;
	...
	public function __construct(){
		...
		$this->load->model('home_slider_model', 'slider');
		$this->data['table'] = 'home_section_slider';
		...
		$this->slider->configure($model_configuration);
	...
	public function list_record(){
		...
		$offset = 20;
		$this->data['main_title'] = 'Home Slider Management';
		$this->data['second_title'] = 'All Slider List';
		...
		$this->data['list'] = $this->slider->getList($srch, $limit, $offset);
		$this->data['list_total'] = $this->slider->getList($srch, $limit, $offset, FALSE);
	...
	public function load_ajax_page(){
		...
		if($page == 'add'){
			...
			$this->data['detail'] = $this->slider->getDetail($id);
	...
	public function edit(){
		...
		if(post() && $this->input->is_ajax_request()){
			...
			if($this->form_validation->run()){
				...
				$ID = post('ID');
				unset($post['ID']);
				$update = $this->slider->updateRecord($post, $ID);
				$this->api->cmd('reload');
			}else{
				$errors = validation_errors_array();
				$this->api->set_error($errors);
